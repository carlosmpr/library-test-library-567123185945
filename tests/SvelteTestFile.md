...
5. **isValidFile(file)**:
   - Checks if the file has a valid extension based on the allowedExtensions array.
...
- **{#if}** and **{#each}**: Svelte conditional and iteration blocks to handle selected files display.
...
>
  ...
  <label for="fileUpload" class="cursor-pointer">
    ...
    {:else}
      <!-- Display message when no files are selected -->
      <div>
